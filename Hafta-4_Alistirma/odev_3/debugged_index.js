...
class ShoppingCart {
    ...
    addItem(productId, quantity = 1) {
        try {
            ...
            product.stock -= quantity; // Stok düşmesi
            this.calculateTotal();
            this.updateUI();
            document.dispatchEvent(new Event('stockUpdate')); // UI güncellemesi
    ...
    removeItem(productId) {
        try {
            ...
            if (product) {
                product.stock += item.quantity; // Stok artması
            }

            this.items.splice(itemIndex, 1);
            this.calculateTotal();
            this.updateUI();
            document.dispatchEvent(new Event('stockUpdate')); // Eksik olan bu satır eklendi
